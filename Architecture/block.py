...
from Architecture.EEM import EEM, ESAM
...
class EMTA_Module(nn.Module):
    def __init__(self, dim, num_heads = 4, bias = True):
        super(EMTA_Module, self).__init__()
        self.num_heads = num_heads
        self.temperature = nn.Parameter(torch.ones(num_heads, 1, 1))
        self.ES = EEM(dim)
        self.qkv = nn.Conv2d(dim, dim * 3, kernel_size=1, bias=bias)
...
class DANetHead(nn.Module):
    def __init__(self, in_channels, out_channels, head_dim=None, num_heads=8, region_size=16):
        ...
        self.MSCASSM = MSCSM(hidden_dim=inter_channels)
        self.ESMD = EMTA_Module(inter_channels)
